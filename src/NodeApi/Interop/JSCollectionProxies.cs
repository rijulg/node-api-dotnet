...
internal static class JSCollectionProxies
{
    ...
    /// <summary>
    /// Creates a proxy handler for a proxy that wraps an <see cref="Array"/>
    /// as a JS Array.
    /// </summary>
    /// <remarks>
    /// The same handler may be used by multiple <see cref="JSProxy"/> instances, for more
    /// efficient creation of proxies.
    /// </remarks>
    internal static JSProxy.Handler CreateArrayProxyHandlerForArray<T>(
        JSValue.From<T> toJS,
        JSValue.To<T> fromJS)
    {
        return new JSProxy.Handler(
            $"{nameof(Array)}<{typeof(T).Name}>")
        {
            Get = (JSObject target, JSValue property, JSObject receiver) =>
            {
                Array array = target.Unwrap<Array>();
                var list = array.OfType<T>().ToList();
